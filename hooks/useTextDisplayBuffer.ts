...
type TextDisplayBuffer = {
  elements: { main: HTMLDivElement | null; shadow: HTMLDivElement | null };
  container: HTMLElement | null;
  switchTimer: ReturnType<typeof setTimeout> | null;
  switching: boolean;
  init: (containerElement: HTMLElement | null) => boolean;
  update: (content: string, style?: CSSProperties) => void;
  cleanup: () => void;
};

function createTextDisplayBuffer(): TextDisplayBuffer {
  const buffer: TextDisplayBuffer = {
    elements: { main: null, shadow: null },
    container: null,
    switchTimer: null,
    switching: false,
    ...
    cleanup() {
      ...
      if (buffer.elements.main?.parentNode) {
        buffer.elements.main.parentNode.removeChild(buffer.elements.main);
      }

      if (buffer.elements.shadow?.parentNode) {
...
type UseTextDisplayBufferParams = {
  text: string;
...
}: UseTextDisplayBufferParams): RefObject<HTMLDivElement | null> {
