...
import { filterPresetStylesForVariableAxes, resolveVariableFontAvailableStyles } from '../utils/fontUtilsCommon';
import {
                ...
                if (blob instanceof Blob && blob.size >= 10_000) {
                    ...
                    if (axes?.length) {
                        const variableAxes = axes.reduce((acc, axis) => {
                            ...
                            const axisName =
                              axis.name &&
                              typeof axis.name === 'object' &&
                              axis.name !== null &&
                              'en' in axis.name
                                ? String((axis.name as { en?: string }).en || tag.toUpperCase())
                                : tag.toUpperCase();
                            acc[tag] = {
                                name: axisName,
                                min: axis.minValue,
                        ...
                        font.variationSettings = buildVariationSettingsCssString(
                          variableAxes as Record<string, { default?: number }>,
                        );
                        changed = true;
                    }
                }
...
/**
 * Fontsource VF: meta API раньше отдавал `variable: true` без осей — догружаем metadata.json с CDN.
 */
async function enrichFontsourceVfAxesFromMetadataIfNeeded(font: SessionFontRecord) {
    if (font?.source !== 'fontsource' || !font.isVariableFont) return;
    const keys =
        font.variableAxes && typeof font.variableAxes === 'object' ? Object.keys(font.variableAxes) : [];
    ...
    try {
        const payload = await fetchFontsourceVariablePackageMetadata(slug);
        const parsed = parseFontsourceVariableAxesFromMeta(payload?.variable);
        if (Object.keys(parsed).length === 0) return;
        font.variableAxes = parsed;
        const hasItalicStyles = Boolean(font.hasItalicStyles);
        font.italicMode = resolveFontsourceItalicMode(parsed, hasItalicStyles);
        await enrichFontsourceVfPresetsFromFileIfNeeded(font);
        font.availableStyles = resolveVariableFontAvailableStyles(font);
        await saveFont(font);
    } catch (e) {
        console.warn('[DB] enrichFontsourceVfAxesFromMetadataIfNeeded:', slug, e);
    }
}

/** Fontsource VF: в IndexedDB мог остаться один «Regular» вместо пресетов по осям. */
async function enrichFontsourceVfPresetsIfNeeded(font: SessionFontRecord) {
    if (font?.source !== 'fontsource' || !font.isVariableFont) return;
    const axisKeys =
        font.variableAxes && typeof font.variableAxes === 'object' ? Object.keys(font.variableAxes) : [];
    if (axisKeys.length === 0) return;
    const current = Array.isArray(font.availableStyles) ? font.availableStyles : [];
    if (current.length > 1) return;

    try {
        await enrichFontsourceVfPresetsFromFileIfNeeded(font);
        const resolved = resolveVariableFontAvailableStyles(font);
        if (resolved.length <= current.length) return;
        font.availableStyles = resolved;
        await saveFont(font);
    } catch (e) {
        console.warn('[DB] enrichFontsourceVfPresetsIfNeeded:', font.name, e);
    }
}

async function enrichFontsourceVfPresetsFromFileIfNeeded(font: SessionFontRecord) {
    if (!(font.file instanceof Blob) || font.file.size === 0) return;
    if (getFontInstanceStyles(font).length > 0) return;
    const slug = String(font.name || '').trim();
    if (!slug) return;
    try {
        const parsed = await parseFontBuffer(await font.file.arrayBuffer(), slug);
        const instances = extractFvarInstanceStyles(parsed);
        if (instances.length === 0) return;
        applyFontInstanceStylesToFont(font, instances);
    } catch (e) {
        console.warn('[DB] enrichFontsourceVfPresetsFromFileIfNeeded:', slug, e);
    }
}
...
async function loadFontFacesForRestoredFont(font) {
    ...
    try {
        ...
        await enrichFontsourceVfAxesFromMetadataIfNeeded(working);
        await enrichFontsourceVfPresetsIfNeeded(working);
        ...
        if (working.isVariableFont && working.variableAxes) {
            initialVarSettings = Object.entries(working.variableAxes).reduce<Record<string, number>>(
              (acc, [tag, axis]) => {
                if (axis && typeof axis === 'object' && 'default' in axis && axis.default !== undefined) {
                  acc[tag] = Number(axis.default);
                }
                return acc;
              },
              {},
...
export function useFontPersistence(
    ...
    handleVariableSettingsChange: ((settings: Record<string, number>, silent?: boolean, font?: SessionFontRecord) => void) | undefined,
    applyPresetStyle: ((presetName: string, font?: SessionFontRecord) => void) | undefined,
            ...
            if (fontToSelect) {
                ...
                setTimeout(() => {
                        ...
                        applyFontViewStateRestorePlan({
                            font: fontToSelect,
                            plan: restorePlan,
                            applyVariableSettings: handleVariableSettingsChange,
                            applyPresetStyle,
                        });
                }, 0);
            }
