...
async function sessionSignIn(loginToken: string, callbackUrl: string): Promise<SignInResponse | undefined> {
...
export default function AuthSignInPage({ isRuGeo = false }: AuthPageProps) {
  ...
  const [credentialsEnabled, setCredentialsEnabled] = React.useState<boolean | null>(null);
  ...
  const finishWithLoginToken = async (loginToken: string) => {
