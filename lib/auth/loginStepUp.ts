import {
  deviceLabelFromUserAgent,
  getDeviceCookieName,
  hashDeviceFingerprint,
  hashDeviceFingerprintWithNetwork,
  newDeviceId,
  parseDeviceIdFromCookie,
} from './deviceFingerprint';
import { sendLoginCodeEmail } from './sendLoginCodeEmail';
import {
  createLoginChallenge,
  isStepUpLoginAvailable,
  isStepUpLoginDisabled,
  isTrustedDevice,
  issueLoginToken,
  touchTrustedDevice,
  trustDevice,
} from './stepUpLogin';
import { matchEnvCredentialsUser } from './envCredentialsLogin';
import { authenticateCredentialsForLogin } from './userStore';

function getClientIp(req) {
  const forwarded = req.headers['x-forwarded-for'];
  if (typeof forwarded === 'string' && forwarded.length > 0) {
    return forwarded.split(',')[0].trim();
  }
  return req.socket?.remoteAddress || '';
}

export function buildLoginDeviceContext(req) {
  const cookieHeader = req.headers.cookie || '';
  const deviceId = parseDeviceIdFromCookie(cookieHeader);
  const userAgent = req.headers['user-agent'] || '';
  const ip = getClientIp(req);
  return { deviceId, userAgent, ip, cookieHeader };
}

export function deviceHashForUser(userId, { deviceId, userAgent, ip }) {
  return hashDeviceFingerprint({ userId, deviceId, userAgent, ip });
}

export async function initiateCredentialsLogin(req, { email, password }) {
  if (!isStepUpLoginAvailable()) {
    throw Object.assign(new Error('Step-up login storage unavailable'), { code: 'UNAVAILABLE' });
  }

  let auth = await authenticateCredentialsForLogin({ email, password });
  if (auth.status === 'invalid') {
    const demoUser = matchEnvCredentialsUser({ email, password });
    const allowEnvDemo =
      demoUser &&
      (isStepUpLoginDisabled() || process.env.NODE_ENV === 'development');
    if (allowEnvDemo) {
      auth = { status: 'ok', user: demoUser };
    } else {
      return { status: 'invalid' };
    }
  }
  if (auth.status === 'unverified') {
    return { status: 'unverified', email: auth.email };
  }
  if (auth.status === 'deleted_recoverable') {
    return {
      status: 'deleted_recoverable',
      email: auth.email,
      recoverableUntil: auth.recoverableUntil,
    };
  }

  const user = auth.user;
  const device = buildLoginDeviceContext(req);
  const deviceHash = deviceHashForUser(user.id, device);

  if (isStepUpLoginDisabled()) {
    const loginToken = await issueLoginToken(user.id);
    return {
      status: 'token',
      loginToken,
      newDeviceId: device.deviceId || newDeviceId(),
    };
  }

  let trusted = await isTrustedDevice({ userId: user.id, deviceHash });
  if (!trusted && device.deviceId) {
    const legacyHash = hashDeviceFingerprintWithNetwork({
      userId: user.id,
      deviceId: device.deviceId,
      userAgent: device.userAgent,
      ip: device.ip,
    });
    if (legacyHash !== deviceHash && (await isTrustedDevice({ userId: user.id, deviceHash: legacyHash }))) {
      await trustDevice({
        userId: user.id,
        deviceHash,
        deviceLabel: deviceLabelFromUserAgent(device.userAgent),
      });
      trusted = true;
    }
  }
  if (trusted) {
    await touchTrustedDevice({ userId: user.id, deviceHash });
    const loginToken = await issueLoginToken(user.id);
    return {
      status: 'token',
      loginToken,
      newDeviceId: device.deviceId || newDeviceId(),
    };
  }

  const { challengeId, codePlain } = await createLoginChallenge({
    userId: user.id,
    deviceHash,
  });

  try {
    await sendLoginCodeEmail({
      to: user.email,
      name: user.name,
      code: codePlain,
    });
  } catch (e) {
    if (e?.code === 'EMAIL_FAILED') {
      return { status: 'email_failed', email: user.email };
    }
    throw e;
  }

  return {
    status: 'needs_code',
    challengeId,
    email: user.email,
    newDeviceId: device.deviceId || newDeviceId(),
  };
}

/** После подтверждения email или успешного входа по коду — не спрашивать код снова с этого браузера. */
export async function trustDeviceForRequest(req, userId) {
  const uid = String(userId || '').trim();
  if (!uid || !isStepUpLoginAvailable() || isStepUpLoginDisabled()) {
    return { newDeviceId: newDeviceId() };
  }
  const device = buildLoginDeviceContext(req);
  const deviceHash = deviceHashForUser(uid, device);
  await trustDevice({
    userId: uid,
    deviceHash,
    deviceLabel: deviceLabelFromUserAgent(device.userAgent),
  });
  return { newDeviceId: device.deviceId || newDeviceId(), deviceHash };
}

export async function completeLoginChallenge(req, { challengeId, code }) {
  const { verifyLoginChallenge } = await import('./stepUpLogin');
  const device = buildLoginDeviceContext(req);
  const result = await verifyLoginChallenge({ challengeId, code });
  const loginToken = await issueLoginToken(result.userId);

  if (result.deviceHash) {
    await trustDevice({
      userId: result.userId,
      deviceHash: result.deviceHash,
      deviceLabel: deviceLabelFromUserAgent(device.userAgent),
    });
  }

  return { loginToken, newDeviceId: device.deviceId || newDeviceId() };
}

export function deviceCookieHeader(deviceId) {
  const maxAge = 30 * 24 * 60 * 60;
  const secure = process.env.NODE_ENV === 'production' ? '; Secure' : '';
  return `${getDeviceCookieName()}=${encodeURIComponent(deviceId)}; Path=/; Max-Age=${maxAge}; HttpOnly; SameSite=Lax${secure}`;
}

export { isStepUpLoginAvailable, isStepUpLoginDisabled, issueLoginToken } from './stepUpLogin';
