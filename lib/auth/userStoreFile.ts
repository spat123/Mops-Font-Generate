...
import { buildDeletedRecoverableError } from './userStoreAccountLifecycle';
...
export async function createCredentialsUser({ name, email, password }) {
  ...
  if (existing) {
    if (existing.deletedAt) {
      if (isAccountRecoverable(existing)) throw buildDeletedRecoverableError(mapUser(existing));
      const idx = users.findIndex((u) => u.id === existing.id);
      if (idx >= 0) users.splice(idx, 1);
    } else if (isEmailVerified(existing)) {
...
export async function upsertOAuthUser({ provider, providerAccountId, email, name, image }) {
  ...
  if (byAccount) {
    if (byAccount.deletedAt) throw buildDeletedRecoverableError(mapUser(byAccount));
  ...
  if (byEmail) {
    if (byEmail.deletedAt) throw buildDeletedRecoverableError(mapUser(byEmail));
...
export async function softDeleteUserById(userId) {
  ...
  const users = await readUsers();
  const user = users.find((u) => String(u?.id || '').trim() === uid);
  if (!user) throw Object.assign(new Error('User not found'), { code: 'NOT_FOUND' });
  if (!user.deletedAt) {
...
export async function issuePasswordResetForEmail(email) {
  ...
  const users = await readUsers();
  const user = users.find((u) => normalizeEmail(u?.email) === e && String(u?.provider || '') === 'credentials');
  if (!user || !isEmailVerified(user) || user.deletedAt || !user.passwordHash) return null;

  const { plain, hash, expiresAt } = createPasswordResetToken();
  user.passwordResetTokenHash = hash;
  user.passwordResetExpiresAt = expiresAt;
  await writeUsers(users);
  return { user: mapUser(user), resetToken: plain };
}
...
export async function resetPasswordByToken(plainToken, newPassword) {
  ...
  const hash = hashVerificationToken(plainToken);
  const users = await readUsers();
  const user = users.find((u) => String(u?.passwordResetTokenHash || '') === hash) || null;
  if (!user) throw Object.assign(new Error('Invalid token'), { code: 'INVALID_TOKEN' });
...
export async function restoreCredentialsAccount({ email, password }) {
  ...
  if (!isAccountRecoverable(user)) throw Object.assign(new Error('Recovery period expired'), { code: 'DELETED_EXPIRED' });
  const ok = await verifyPassword(p, { salt: user.passwordSalt, hash: user.passwordHash });
