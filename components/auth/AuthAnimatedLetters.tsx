...
type AuthAxisMeta = { min: number; max: number; defaultValue?: number };
type AxesByTag = Record<string, AuthAxisMeta>;
type CenterPhase = 'upper' | 'lower' | 'hidingLower' | 'hidingUpper';

let fontLoadPromise: Promise<void> | null = null;
let axesPromise: Promise<AxesByTag> | null = null;
 ...
 */
function buildAuthAxisPicker(axesByTag: AxesByTag | null | undefined, role: 'upper' | 'lower') {
  const pick = (tag: string, fallback: AuthAxisMeta | null) => {
    const axis = axesByTag?.[tag];
    if (!axis) return fallback;
    return axis;
  };

  const niceRanges: Record<string, (a: AuthAxisMeta) => { min: number; max: number }> = {
...
function buildAlphabet(isRuGeo: boolean) {
  if (isRuGeo) {
...
export function AuthAnimatedLetters({ isRuGeo = false }) {
  ...
  const [centerPhase, setCenterPhase] = useState<CenterPhase>(reducedMotion ? 'lower' : 'upper');

  const [axesByTag, setAxesByTag] = useState<AxesByTag | null>(null);
  ...
  const fsmTimersRef = useRef<number[]>([]);
  const axesTimersRef = useRef<number[]>([]);
