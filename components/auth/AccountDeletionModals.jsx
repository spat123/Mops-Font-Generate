...
function ModalShell({ open, title, onClose, children, closeLabel = 'Закрыть' }) {
    ...
    >
      <div ref={panelRef} className="w-full max-w-md overflow-hidden bg-white shadow-xl">
        ...
        <div className="border-t border-gray-100 px-6 py-4">
          <AppButton type="button" variant="primary" size="md" className="w-full" onClick={onClose}>
            {closeLabel}
          </AppButton>
        </div>
      </div>
...
export function AccountDeleteConfirmModal({ open, onClose, onConfirm, busy = false }) {
  return (
...
export function AccountDeletedSuccessModal({ open, onClose, recoverableUntil }) {
