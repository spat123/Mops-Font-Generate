...
type VariableAxisDef = { default?: number; min?: number; max?: number };
type GenAxisSettings = Record<string, number>;
...
export default function GenerateFontModal({
  ...
  const [genSettings, setGenSettings] = useState<GenAxisSettings>({});
  ...
  const axisRows = useMemo(() => {
    ...
    return Object.entries(axes)
      ...
      .map(([tag, axis]) => {
        const axisDef = axis as VariableAxisDef;
        const raw = settings[tag];
        const fallback = axisDef?.default ?? axisDef?.min ?? 0;
      ...
      if (blob) {
        const filename =
          fileSubfamily && fileSubfamily.toLowerCase() !== 'regular'
            ...
            : `${base}.${format}`;
        downloadFile(blob, filename, mimeTypeForFontExt(format));
          ...
          <div className="space-y-4">
            ...
            <div className="relative min-w-0 w-full">
              <CustomSelect
                ...
                onChange={(v) => setSubfamilyPreset(pickSelectValue(v))}
              ...
              <div className="flex min-w-0 w-full gap-3">
                ...
                <div className="min-w-[6.5rem]">
                  <CustomSelect
                    ...
                    onChange={(v) => setCustomWeightClass(pickSelectValue(v))}
                    disabled={inputInactive || nameReadOnly}
            ...
            <div>
              <CustomSelect
                ...
                className={formatClass}
                value={format}
                onChange={(v) => setFormat(pickSelectValue(v))}
                disabled={disabled}
