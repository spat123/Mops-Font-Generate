...
import { IconCircleButton, type IconCircleButtonVariant } from './IconCircleButton';
import { useDismissibleLayer } from './useDismissibleLayer';

export type CardActionMenuSubItem = {
  key: string;
  label: string;
  disabled?: boolean;
  icon?: ReactNode;
...
export type CardActionMenuItem = {
  ...
  icon?: ReactNode;
  onSelect?: () => void;
  submenuItems?: CardActionMenuSubItem[];
  /** Визуальный акцент пункта (например, удаление). */
  tone?: 'danger' | 'light';
};
...
export type CardActionsMenuProps = {
  ...
  triggerLabel?: string;
  triggerVariant?: IconCircleButtonVariant;
  className?: string;
...
function suppressMenuPointerPassthrough(event: MouseEvent) {
...
export function CardActionsMenu({
  ...
  const [menuPosition, setMenuPosition] = useState<FloatingMenuPosition>({ top: 0, left: 0, ready: false });
  const [submenuPosition, setSubmenuPosition] = useState<FloatingMenuPosition>({
    top: 0,
    left: 0,
    ready: false,
  });
  ...
  const openSubmenuForItem = (itemKey: string, targetEl: HTMLElement) => {
  ...
  const scheduleSubmenuClose = (itemKey: string) => {
  ...
  const activateLeafMenuItem = (event: MouseEvent, onSelect?: () => void) => {
