...
export type VirtualizedGlyphGridProps = {
  ...
  columnCount?: number;
  rowGapPx?: number;
  seamlessGrid?: boolean;
  onInnerWidth?: (width: number) => void;
  onVisibleIndexRangeChange?: (range: { startIndex: number; endIndex: number }) => void;
};

function columnsFromWidth(w: number) {
  if (w >= 1280) return 10;
  if (w >= 1024) return 8;
  if (w >= 768) return 6;
  if (w >= 640) return 4;
...
export function VirtualizedGlyphGrid({
  ...
  const [widthHostEl, setWidthHostEl] = useState<HTMLDivElement | null>(null);

  const setWidthHostRef = useCallback((node: HTMLDivElement | null) => {
    setWidthHostEl(node);
  }, []);
  ...
  const cells = useMemo(() => {
    ...
    for (let i = range.start; i < range.end; i++) {
      out.push(<Fragment key={i}>{renderItem(i)}</Fragment>);
    }
    return out;
