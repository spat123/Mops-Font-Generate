 ...
 * - `toolbarIcon` — как круглые `IconCircleButton` variant `toolbar`, но `rounded-md` (нижняя полоса сайдбара): серый фон, иконка `text-gray-800`, hover — accent + белый текст; `pressed` — открытое состояние.
 * - `soft` — серый фон, hover: лёгкий accent; `pressed` — активное красное (как кнопка настроек в футере).
 * - `chip` — белая с рамкой, hover: чёрная заливка; `pressed` — выбранное состояние accent (чипы/пресеты в сайдбаре).
 * - `link` — текстовая, без рамки; hover: accent + подчёркивание.
 *
 * Размеры: `rail` — ровно 36px по высоте (футер сайдбара: `p-2` + ряд = 52px), без дробных rem от `md`.
 *
...
const SIZE_CLASS = {
  ...
  // Ровно 36px: ряд в футере сайдбара при p-2 → 52px без дробных rem
...
function resolveVariantClass(variant, pressed) {
  if (variant === 'soft') {
