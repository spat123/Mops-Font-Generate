...
export type IconRailSegmentOpts = {
  hasIcon?: boolean;
  disabled?: boolean;
};

function optionTipText(opt: SegmentedControlOption): string {
  if (opt.title) return opt.title;
  if (typeof opt.label === 'string') return opt.label;
  return '';
}

function optionAriaLabel(opt: SegmentedControlOption): string {
  return opt['aria-label'] ?? optionTipText(opt);
}
...
function IconViewPlain(props: SVGProps<SVGSVGElement>) {
  return (
    <svg
...
function IconViewWaterfall(props: SVGProps<SVGSVGElement>) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="16"
...
function IconViewGlyphs(props: SVGProps<SVGSVGElement>) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="16"
...
function IconViewStyles(props: SVGProps<SVGSVGElement>) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="16"
 ...
 */
export function iconRailSegmentClass(active: boolean, opts: IconRailSegmentOpts = {}) {
...
export function SegmentedControl<T extends string = string>({
  ...
  const shouldShowTooltip = (opt: SegmentedControlOption<T>) =>
      ...
      >
        {options.map((opt) => {
          ...
          return showTip ? (
            ...
            <Fragment key={opt.value}>{btn}</Fragment>
          );
        })}
      </div>
    );
