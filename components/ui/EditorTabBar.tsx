...
type CloseButtonMode = 'active-always' | 'active-hover' | 'all-hover';
...
type TabDensityClasses = {
  tabWidthClass: string;
  tabMinWidthClass: string;
  tabMaxWidthClass: string;
  placeholderWidthClass: string;
  placeholderMinWidthClass: string;
  placeholderMaxWidthClass: string;
};

type FontTabPlaceholder = {
  id: string;
  label: string;
};
...
const tabBg = (active: boolean) => (active ? 'bg-accent' : 'bg-white');
const tabBottom = (active: boolean) => (active ? '' : INACTIVE_TAB_BOTTOM);

const TAB_BASE = `flex min-w-0 shrink-0 items-stretch ${BORDER_R}`;

function tabShellWithClose(active: boolean, minWClass = 'min-w-[8rem]') {
...
function getFontTabSourceMeta(source: string | undefined): SourceBadgeMeta | null {
  if (source === 'local') return { shortLabel: 'L', fullLabel: 'Локальный' };
  if (source) return { shortLabel: 'D', fullLabel: 'Dynamic Font' };
  return null;
}

function getCloseButtonMode(totalTabCount: number): CloseButtonMode {
  if (totalTabCount >= 9) return 'active-always';
  if (totalTabCount >= 6) return 'active-hover';
  return 'all-hover';
}

function closeBtnClass(onAccent: boolean, mode: CloseButtonMode = 'all-hover') {
...
function shouldHideCloseButtonFromA11y(active: boolean, mode: CloseButtonMode = 'all-hover') {
  if (mode === 'all-hover') return false;
  return !active;
}

function closeButtonTabIndex(active: boolean, mode: CloseButtonMode = 'all-hover') {
  if (mode === 'all-hover') return 0;
  return active ? 0 : -1;
}

function labelTextClass(active: boolean) {
  return `min-w-0 flex-1 truncate text-center text-xs leading-none ${
    active ? 'text-white' : ' text-gray-900 group-hover:text-accent'
  }`;
}

function sourceBadgeClass(active: boolean) {
  return [
...
function getTabDensityClasses(totalTabCount: number): TabDensityClasses {
  if (totalTabCount >= 9) {
    return {
...
function scrollArrowButtonClass(disabled = false): string {
  return [
...
function ScrollArrowIcon({ direction = 'left' }: { direction?: 'left' | 'right' }) {
...
const LIBRARY_LABEL_BTN = (active: boolean) =>
...
function ClosableEditorTab({
  ...
  active: boolean;
  label: string;
  sourceBadge?: SourceBadgeMeta | null;
  title?: string;
  ariaTabLabel: string;
  onSelect: () => void;
  onRemove?: () => void;
 ...
 */
export type EditorTabBarProps = {
  mainTab: string;
  emptySlotIds?: string[];
  emptySlotLabelsById?: Record<string, string>;
  fonts?: SessionFontRecord[];
  fontTabPlaceholders?: FontTabPlaceholder[] | null;
  showNewTabSsrFallback?: boolean;
  onLibraryClick: () => void;
  onEmptyTabClick: (slotId: string) => void;
  onRemoveEmptySlot?: (slotId: string) => void;
  onFontClick: (font: SessionFontRecord) => void;
  onRemoveFont?: (fontId: string) => void;
  onAddEmptySlot: () => void;
  fontTabCloseAriaLabel?: string;
