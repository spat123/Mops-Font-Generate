...
export type SquareToastKind = 'success' | 'loading' | 'error';

export type SquareToastProps = {
  kind?: SquareToastKind;
  ariaLabel?: string;
};

export function SquareToast({ kind = 'success', ariaLabel }: SquareToastProps) {
  const label =
    ariaLabel || (kind === 'loading' ? 'Загрузка' : kind === 'error' ? 'Ошибка' : 'Готово');
