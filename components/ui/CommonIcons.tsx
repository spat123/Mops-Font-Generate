...
export function PlusIcon({ className = 'h-4 w-4' }: IconClassNameProps) {
  return (
    <svg
...
export function EditIcon({ className = 'h-4 w-4' }: IconClassNameProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
...
export function TrashIcon({ className = 'h-4 w-4' }: IconClassNameProps) {
  return <EditAssetIcon src={delIconUrl} className={className} />;
}

/** Открыть во внешнем / в редакторе */
export function OpenExternalIcon({ className = 'h-4 w-4' }: IconClassNameProps) {
