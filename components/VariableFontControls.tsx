...
export default function VariableFontControls({ font, onSettingsChange, isAnimating = false, toggleAnimation }) {
  const [axes, setAxes] = useState<VariableAxisDef[]>([]);
  ...
  useEffect(() => {
    ...
    if (font.variableAxes && Object.keys(font.variableAxes).length > 0) {
      ...
      const fontAxes = Object.entries(font.variableAxes).map(([tag, axisDataRaw]) => {
        const axisData = axisDataRaw as VariableAxisDef;
        const name = axisData.name || tag;
      ...
      if (filteredAxes.length > 0) {
        ...
        if (fontId !== loadedFontId.current) {
          ...
          const initialDirections: Record<string, number> = {};
    ...
    const loadFontAxes = async () => {
      try {
        ...
        const fontAxes = (await getVariableAxes(font)) as VariableAxisDef[];
  ...
  return (
    <div className="space-y-4">
      ...
      {axesForDisplay.map(axis => {
        ...
        const axisName =
          axis.name != null && typeof axis.name === 'object'
            ? ((axis.name as { en?: string }).en || Object.values(axis.name as object)[0] || axis.tag)
            : (axis.name || axis.tag);
