...
import {
  parseFontVariationSettingsString,
  stringifyFontVariationEntries,
  upsertFontVariationEntry,
} from '../utils/fontVariationSettings';
...
export type WaterfallModeProps = {
  waterfallSizes?: number[];
  scrollParentRef?: RefObject<HTMLElement | null>;
  isVariableFontAnimating?: boolean;
  isInteractingWithWaterfallSize?: boolean;
};
  ...
  const renderItem = useCallback(
    (index) => {
      ...
      if (selectedFont?.isVariableFont) {
        ...
        let fvs = parseFontVariationSettingsString(baseTextStyle?.fontVariationSettings);
        if (axes.wght !== undefined) {
          ...
          fvs = upsertFontVariationEntry(fvs, 'wght', rowWght);
        }
        if (axes.ital !== undefined) {
          fvs = upsertFontVariationEntry(fvs, 'ital', presetInfo?.style === 'italic' ? 1 : 0);
        }
        if (axes.slnt !== undefined) {
          fvs = upsertFontVariationEntry(
            fvs,
            'slnt',
            presetInfo?.style === 'italic' ? (axes.slnt?.min ?? -15) : (axes.slnt?.default ?? 0),
          );
        }
        itemStyle.fontVariationSettings = stringifyFontVariationEntries(fvs);
