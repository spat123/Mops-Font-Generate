...
export type CatalogCheckboxMarkProps = {
  checked?: boolean;
  inverted?: boolean;
  className?: string;
};
...
export type CatalogCheckboxControlProps = {
  checked?: boolean;
...
export function CatalogCheckboxControl({
  ...
  return (
    ...
    >
      ...
      <span className={['min-w-0 whitespace-nowrap', labelClassName].filter(Boolean).join(' ')}>{label}</span>
    </label>
  );
}
