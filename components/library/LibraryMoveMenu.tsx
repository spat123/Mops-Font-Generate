...
export type LibraryMoveMenuProps = {
  disabled?: boolean;
  hasSelection?: boolean;
  busy?: boolean;
  libraries?: SavedLibraryRecord[];
  currentLibraryId?: string | null;
  onMoveToLibrary?: (targetLibraryId: string) => void | Promise<void>;
...
export function LibraryMoveMenu({
  ...
  onMoveToLibrary,
  onCreateLibrary,
}: LibraryMoveMenuProps) {
  const { assertCanCreateNewLibrary, isAuthenticated, canCreateNewLibrary, openPlans } = useLibraryAuth();
  const [open, setOpen] = useState(false);
  const [pendingTargetLibraryId, setPendingTargetLibraryId] = useState<string | null>(null);
