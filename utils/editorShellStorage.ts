...
/** Вкладки шрифтов из библиотеки, закрытые крестиком — восстановление после F5. */
...
/** До useLayoutEffect не подсвечиваем «Все шрифты» / не показываем контент — убирает мигание. */
export const EDITOR_MAIN_TAB_PENDING = '__editorShellPending__';

export type EditorShellStorageSnapshot = {
  emptySlotIds: string[];
  mainTab: string;
};

export function newEmptySlotId(): string {
...
export function readEditorShellFromStorage(): EditorShellStorageSnapshot | null {
  ...
  try {
    ...
    let emptySlotIds: string[];
    if (rawSlots !== null) {
      const p: unknown = JSON.parse(rawSlots);
      emptySlotIds = Array.isArray(p)
...
export function isFontTabId(tab: string | null | undefined): boolean {
  return (
    typeof tab === 'string' &&
    tab !== 'library' &&
