...
export function getFreePlanFeatures(locale: BillingLocale | string = 'RU'): string[] {
  ...
  if (en) {
    return [
      ...
      'Preview modes: Plain, Type Scale, Glyphs, Styles',
      'Type Scale: preset scale ratios (custom scale — Pro)',
  ...
  return [
    ...
    'Режимы превью: Plain, Type Scale, Glyphs, Styles',
    'Type Scale: готовые шкалы (своё значение — Pro)',
...
export function getProPlanFeatures(locale: BillingLocale | string = 'RU'): string[] {
  ...
  if (en) {
    return [
      ...
      'Type Scale: custom scale ratio (any value 1.001–3)',
  ...
  return [
    ...
    'Type Scale: своё значение шкалы (коэффициент 1.001–3)',
