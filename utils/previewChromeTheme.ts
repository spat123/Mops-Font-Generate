...
export function parseColorToRgba(input: unknown): RgbaColor | null {
...
export function compositeRgbOverBackdrop(rgba: RgbaColor, backdrop: RgbColor = { r: 255, g: 255, b: 255 }): RgbColor {
...
export function getPreviewChromeFromBackground(backgroundColor: string): PreviewChromeTheme {
  const parsed = parseColorToRgba(backgroundColor);
  const backdrop: RgbColor = { r: 255, g: 255, b: 255 };
  let rgb: RgbColor = backdrop;
  if (parsed) {
  ...
  return {
    ...
    noteBox: 'p-4',
    noteText: 'text-sm text-gray-600',
