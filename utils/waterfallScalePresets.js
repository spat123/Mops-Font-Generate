...
export const WATERFALL_SCALE_PRESETS = [
...
export function findWaterfallScalePresetByRatio(ratio) {
  const r = Number(ratio);
  if (!Number.isFinite(r)) return null;
  return WATERFALL_SCALE_PRESETS.find((p) => Math.abs(p.ratio - r) <= PRESET_RATIO_EPS) || null;
}

export function getWaterfallScaleSelectKeyForRatio(ratio) {
  const hit = findWaterfallScalePresetByRatio(ratio);
  return hit ? hit.key : 'custom';
}

export function isWaterfallCustomScaleRatio(ratio) {
