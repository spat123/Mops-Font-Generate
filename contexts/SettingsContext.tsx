...
const DEFAULT_SETTINGS = {
  ...
  WATERFALL_EDIT_TARGET: 'heading' as WaterfallEditTarget,
  ...
  WATERFALL_UNIT: 'px' as WaterfallUnit,
...
function coerceTextDecoration(value: unknown): TextDecoration {
  if (value === 'underline' || value === 'line-through') return value;
  return 'none';
}

function coerceListStyle(value: unknown): ListStyle {
  if (value === 'bullet' || value === 'numbered') return value;
  return 'none';
}

function coerceWaterfallEditTarget(value: unknown): WaterfallEditTarget {
  return value === 'body' ? 'body' : 'heading';
}

function coerceWaterfallUnit(value: unknown): WaterfallUnit {
  if (value === 'rem' || value === 'pt') return value;
  return 'px';
}

function coerceVerticalAlignment(value: unknown): VerticalAlignment {
...
export function getDefaultPreviewSettingsSnapshot(): PreviewSettingsSnapshot {
  return {
...
function useSyncSettingToStorage(isClient: boolean, storageKey: string, value: unknown) {
  useEffect(() => {
    if (!isClient) return;
...
function useDebouncedSyncSettingToStorage(
  isClient: boolean,
  storageKey: string,
  value: unknown,
  delayMs = 180,
...
export const SettingsProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [textDecoration, setTextDecoration] = useState<TextDecoration>(DEFAULT_SETTINGS.TEXT_DECORATION);
  const [listStyle, setListStyle] = useState<ListStyle>(DEFAULT_SETTINGS.LIST_STYLE);
  ...
  const [waterfallEditTarget, setWaterfallEditTarget] = useState<WaterfallEditTarget>(
    DEFAULT_SETTINGS.WATERFALL_EDIT_TARGET,
  );
  ...
  const [waterfallUnit, setWaterfallUnit] = useState<WaterfallUnit>(DEFAULT_SETTINGS.WATERFALL_UNIT);
  ...
  const [verticalAlignment, setVerticalAlignment] = useState<VerticalAlignment>(
    DEFAULT_SETTINGS.VERTICAL_ALIGNMENT,
  );
  ...
  const setDarkTheme = useCallback((valueOrUpdater: boolean | ((prev: boolean) => boolean)) => {
      const nextValue =
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && typeof window.matchMedia === 'function') {
      ...
      const handleMediaChange = (event: MediaQueryListEvent) => setSystemPrefersDark(event.matches);
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    setTextDecoration(
      coerceTextDecoration(getLocalStorageItem(LOCAL_STORAGE_KEYS.TEXT_DECORATION, DEFAULT_SETTINGS.TEXT_DECORATION)),
    );
    setListStyle(coerceListStyle(getLocalStorageItem(LOCAL_STORAGE_KEYS.LIST_STYLE, DEFAULT_SETTINGS.LIST_STYLE)));
    setTextColumns(
    ...
    let va: VerticalAlignment = DEFAULT_SETTINGS.VERTICAL_ALIGNMENT;
